...
execution, file transfer with space checks, and streaming support.
"""
...
class SSHConfig:
    ...
    Attributes:
        ...
        security: Security configuration for this connection.
        compression: Enable SSH compression.
        host_key_callback: Custom host key verification callback.
    """
    ...
    proxy: Optional[str] = None  # ProxyCommand string or jump host for asyncssh proxy support

    def validate(self) -> None:
        """Validate all configuration parameters.

        Raises:
            AIOSSHInvalidParameterError: If any parameter is invalid.
        """
...
class FastSSHSession:
    """High-performance async SSH session.

    Provides secure command execution, file transfer, and streaming
    with comprehensive error handling and concurrent execution support.

    Example:
        config = SSHConfig(host="server.com", username="admin", password="secret")
        async with FastSSHSession(config) as session:
    ...
    def __init__(self, config: SSHConfig) -> None:
        ...
        Raises:
            AIOSSHInvalidParameterError: If config is invalid.
        """
        config.validate()
        self.config = config
    ...
    def connection(self) -> Optional["SSHClientConnection"]:
        """Return the underlying asyncssh SSHClientConnection for advanced use
        (e.g. tunneling with SSHTunnelManager or create_tunnel).
        """
    ...
    async def connect(self) -> None:
        ...
        Maps asyncssh exceptions to specific AIOSSH exceptions for
        precise error handling by callers.

        Raises:
            AIOSSHConnectionTimeoutError: Connection timed out.
            AIOSSHConnectionRefusedError: Server refused connection.
            AIOSSHInvalidCredentialsError: Authentication failed.
            AIOSSHHostKeyVerificationError: Host key not trusted.
            AIOSSHConnectionError: Other connection errors.
            AIOSSHSessionExpiredError: Session permanently closed.
        """
        async with self._lock:
            ...
            if self._closed:
                raise AIOSSHSessionExpiredError(
                    "Session has been permanently closed",
                    code="SESSION_CLOSED",
                )
            ...
            try:
                connect_kwargs: dict[str, object] = {
                    ...
                    "known_hosts": (
                        self.config.host_key_callback
                        or self._default_host_key_handler
                    ),
                ...
                raise AIOSSHConnectionRefusedError(
                    f"Server refused connection: {e!s}",
                    code="DISCONNECTED",
                    details={"host": self.config.host, "reason": str(e)},
                    cause=e,
                )
                ...
                raise AIOSSHConnectionError(
                    f"DNS resolution failed: {e!s}",
                    code="DNS_ERROR",
                    details={"host": self.config.host},
                    cause=e,
                )
            except AIOSSHConnectionError:
                raise
    ...
    @staticmethod
    def _default_host_key_handler(
        host: str, port: int, key: object
    ) -> bool:
        """Default host key handler - accepts all keys.

        Override with a custom callback in production for security.

        Args:
            host: Hostname being connected to.
            port: Port being connected to.
    ...
    async def execute(
        ...
        *,
        ...
        Args:
            ...
            sudo: If True, prefix with 'sudo -n'.
            allow_dangerous: If True, skip dangerous command checks.

        Returns:
            Dictionary with keys: command, stdout, stderr, exit_code,
            success, execution_time.

        Raises:
            AIOSSHSessionExpiredError: If session is not connected.
            AIOSSHCommandTimeoutError: If command times out.
            AIOSSHCommandError: If command execution fails.
        """
        if not self.is_connected:
            raise AIOSSHSessionExpiredError(
                "Cannot execute command: session not connected",
        ...
        async with self._command_semaphore:
            try:
                ...
                result: SSHCompletedProcess = await asyncio.wait_for(
                    self._connection.run(command, check=False),  # type: ignore[union-attr]
                    timeout=timeout,
                )
                ...
                # Safe exit code handling - asyncssh may return None, int, or other types
                ...
                return {
                    ...
                    "truncated": (
                        result.stdout_was_truncated or result.stderr_was_truncated
                    ),
                }
    ...
    async def execute_batch(
        ...
        **kwargs: object,
        ...
        """Execute multiple commands with optional parallelism.

        Args:
            commands: List of commands to execute.
            parallel: Execute commands concurrently if True.
            max_concurrent: Maximum concurrent commands when parallel.
            **kwargs: Additional arguments passed to execute().

        Returns:
            List of result dictionaries (same order as input).
        """
        if not commands:
            return []
        ...
        if parallel:
            ...
            async def _bounded_execute(cmd: str) -> dict[str, object]:
                async with semaphore:
                    try:
                        return await self.execute(cmd, **kwargs)  # type: ignore[arg-type]
                    except Exception as e:
                        return {
                            "command": cmd,
                            "error": str(e),
            ...
            for cmd in commands:
                try:
                    result = await self.execute(cmd, **kwargs)  # type: ignore[arg-type]
    ...
    async def upload_file(
        ...
        *,
        ...
        Args:
            ...
            check_disk_space: Verify remote disk space before upload.

        Returns:
            Dictionary with upload results including transfer speed.

        Raises:
            AIOSSHSessionExpiredError: If session is not connected.
            AIOSSHFileTransferNotFoundError: If local file not found.
            AIOSSHFileUploadError: If upload fails.
        """
        if not self.is_connected:
            raise AIOSSHSessionExpiredError(
                "Cannot upload: session not connected",
                code="SESSION_NOT_CONNECTED",
            )
                    ...
                    try:
                        ...
                        if available < file_size:
                            raise AIOSSHFileDiskFullError(
                                ...
                                f"need {file_size}, have {available} bytes",
                                code="DISK_FULL",
                                details={
                                    "needed": file_size,
                                    "available": available,
                        ...
                        pass  # Cannot check, proceed

                await sftp.put(str(local_file), remote_path)

        except AIOSSHFileUploadError:
            raise
        except asyncssh.SFTPError as e:
            raise AIOSSHFileUploadError(
                f"SFTP upload failed: {e!s}",
                code="SFTP_ERROR",
                cause=e,
            )
        except Exception as e:
        ...
        return {
            ...
            "speed_mbps": (
                round((file_size / 1_000_000) / elapsed, 3)
                if elapsed > 0
                else 0.0
            ),
        }
    ...
    async def download_file(
        ...
        *,
        ...
        Args:
            ...
            resume: Resume partial download if True.

        Returns:
            Dictionary with download results including transfer speed.

        Raises:
            AIOSSHSessionExpiredError: If session is not connected.
            AIOSSHFileTransferNotFoundError: If remote file not found.
            AIOSSHFileDownloadError: If download fails.
        """
        if not self.is_connected:
            raise AIOSSHSessionExpiredError(
                "Cannot download: session not connected",
                code="SESSION_NOT_CONNECTED",
            )
        ...
        try:
            async with self._connection.start_sftp_client() as sftp:  # type: ignore[union-attr]
                ...
                return {
                    ...
                    "transfer_time": round(elapsed, 3),
                    "speed_mbps": (
                        round((remote_size / 1_000_000) / elapsed, 3)
                        if elapsed > 0
                        else 0.0
                    ),
                    "resumed": offset > 0,
            ...
            raise AIOSSHFileDownloadError(
                f"SFTP download failed: {e!s}",
                code="SFTP_ERROR",
                cause=e,
            )
        except Exception as e:
            raise AIOSSHFileDownloadError(
                f"Download failed: {e!s}",
                code="DOWNLOAD_ERROR",
                cause=e,
            )

    async def stream_command(self, command: str) -> AsyncIterator[str]:
        ...
        Yields:
            Lines of stdout output as they become available.

        Raises:
            AIOSSHSessionExpiredError: If session is not connected.
            AIOSSHCommandError: If streaming fails.
        """
        if not self.is_connected:
            raise AIOSSHSessionExpiredError(
                "Cannot stream: session not connected",
                code="SESSION_NOT_CONNECTED",
            )
        ...
        try:
            ...
            raise AIOSSHCommandError(
                f"Command streaming failed: {e!s}",
                code="STREAM_ERROR",
                command=command[:200],
                cause=e,
            )
    ...
    async def close(self) -> None:
        ...
        Attempts graceful close with 5-second timeout, then force aborts.
        """
        async with self._lock:
            if self._closed:
                return
