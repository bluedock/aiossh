...
class AIOSSHCommandError(AIOSSHException):
    ...
    Accepts an optional ``command`` keyword (in addition to the base
    exception's fields) so callers can attach the offending command text
    without needing to stuff it into ``details`` manually.
    """
