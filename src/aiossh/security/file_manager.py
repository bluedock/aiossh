...
from .config import SecureMemory
...
class SessionFileManager:
    ...
    def _derive_key(self, master_password: str, salt: bytes) -> bytearray:
        kdf = PBKDF2HMAC(
        ...
        # Return a *mutable* bytearray (not immutable bytes) so callers can
        # actually zero the derived key in place via SecureMemory.secure_clear.
        return bytearray(kdf.derive(master_password.encode("utf-8")))
    ...
    def create_session_file(self, filename: str, credentials: dict[str, Any], master_password: str) -> Path:
        ...
        try:
            plaintext = bytearray(_json_dumps(credentials))
            aesgcm = AESGCM(key)
            ...
            # Wipe the ACTUAL sensitive buffers, not throwaway copies.
            # Previously secure_clear(bytearray(key)) zeroed a *copy*, so the
            # derived key and plaintext credentials were never wiped at all.
