"""
Core AIOSSH Client - v1.1.3 (simplified high-level facade)
...
from ..security.file_manager import SessionFileManager  # type: ignore
...
class AIOSSH:
    """Main high-level async SSH client (v1.1.3)."""
    ...
    def __init__(
        ...
        *,
        ...
        # Tracks sessions that came from the pool, mapped to the SSHConfig
        # used to acquire them, so they can be returned (not hard-closed)
        # when the caller is done with them.
        self._pooled_sessions: dict[FastSSHSession, SSHConfig] = {}
        ...
        if use_pool:
            try:
                ...
                async with self._lock:
                    self._pooled_sessions[session] = config
            except AIOSSHPoolExhaustedError:
                # Previously this was swallowed by the broad `except Exception`
                # below, so once the pool hit max_connections every further
                # call would silently open a brand-new connection completely
                # outside the pool's accounting - defeating the whole point
                # of a "strict max connection enforcement" pool. Propagate
                # it so callers actually see (and can react to) exhaustion.
                raise
            except Exception:
            ...
            if isinstance(session_id, str):
                ...
                for k, v in list(self._active_sessions.items()):
                    if v is session:
                        ...
                        break
            pool_config = self._pooled_sessions.pop(session, None) if session else None
        if session:
            await self._release_session(session, pool_config)
    ...
    async def close_all(self) -> None:
        ...
        async with self._lock:
            ...
            pooled = dict(self._pooled_sessions)
            self._pooled_sessions.clear()
        for s in sessions:
            try:
                await self._release_session(s, pooled.get(s))
            except Exception:
                pass
        await self._pool.close()

    async def _release_session(
        self, session: FastSSHSession, pool_config: Optional[SSHConfig]
    ) -> None:
        """Release a session: return it to the pool if it came from there,
        otherwise close it outright."""
        if pool_config is not None:
            try:
                await self._pool.return_connection(pool_config, session)
                return
            except Exception:
                pass
        try:
