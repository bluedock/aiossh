...
from typing import AsyncIterator, Callable, Optional, Self
...
class SSHConfig:
    ...
    # host_key_callback must be a value accepted by asyncssh's `known_hosts`
    # parameter: a path to a known_hosts file, an SSHKnownHosts object, bytes,
    # or None.  A plain Python callable returning bool is NOT accepted by
    # asyncssh >= 2.x (it causes "'bool' object is not iterable").  Pass None
    # (the default) to disable host-key checking, or supply a proper asyncssh-
    # compatible known_hosts source.  For custom validation logic, subclass
    # asyncssh.SSHClient and override validate_host_public_key().
    host_key_callback: Optional[object] = None
    # proxy: for jump-host / tunnel support pass an existing
    # asyncssh.SSHClientConnection.  asyncssh has no `proxy` string parameter;
    # string values stored here are ignored.  Use SSHTunnelManager for jump
    # hosts when you need to create the tunnel connection first.
...
class FastSSHSession:
    ...
    async def connect(self) -> None:
        async with self._lock:
            ...
            try:
                # ── Build connect kwargs compatible with asyncssh >= 2.14 ─────
                connect_kwargs: dict[str, object] = {
                    ...
                    # BUG FIX: asyncssh >= 2.14 renamed the parameter from
                    # `compression` (bool) to `compression_algs` (list|None).
                    # Passing the old name raises TypeError.
                    "compression_algs": (
                    ...
                    # BUG FIX: asyncssh's `known_hosts` does NOT accept a plain
                    # Python callable that returns bool.  Passing one (or True)
                    # causes "'bool' object is not iterable" because asyncssh
                    # tries to load the value as a known-hosts source.
                    # `None` is the correct way to tell asyncssh to skip host-key
                    # verification entirely (equivalent to the old always-return-
                    # True handler).  If the caller supplied a real asyncssh-
                    # compatible known_hosts value it is forwarded as-is.
                    "known_hosts": (
                        self.config.host_key_callback
                        if self.config.host_key_callback is not None
                        else None
                    ),

                    # keepalive_interval is valid in asyncssh >= 2.x.
                ...
                # BUG FIX: asyncssh has no `proxy` parameter.  For jump-host /
                # tunnel support asyncssh uses a `tunnel` parameter that must
                # receive an existing SSHClientConnection, not a string.
                # If the caller stored an SSHClientConnection in config.proxy,
                # forward it as `tunnel`.  String values (legacy ProxyCommand
                # strings) are silently ignored here — use SSHTunnelManager
                # for jump-host support instead.
                if self.config.proxy is not None and isinstance(
                    self.config.proxy, SSHClientConnection
                ):
                    connect_kwargs["tunnel"] = self.config.proxy

                # BUG FIX: `connect_timeout` is not a recognised keyword in all
                # asyncssh versions.  We already wrap the call in
                # asyncio.wait_for() below, which gives us the same guarantee.
                # Keeping both caused "unexpected keyword argument" errors on some
                # builds.  The outer wait_for timeout is the authoritative one.
    ...
    def _default_host_key_handler(host: str, port: int, key: object) -> bool:
        """Legacy helper — kept for backward compatibility but no longer passed
        to asyncssh internally.

        asyncssh >= 2.x does not accept a plain bool-returning callable as
        ``known_hosts``; passing one raises "'bool' object is not iterable".
        The default behaviour (accept all host keys without checking) is now
        achieved by passing ``known_hosts=None`` to asyncssh.connect().

        In production, supply a proper asyncssh-compatible ``known_hosts``
        value in ``SSHConfig.host_key_callback`` (e.g. a path to a known_hosts
        file) or subclass ``asyncssh.SSHClient`` and override
        ``validate_host_public_key()`` to prevent MITM attacks.
        """
        return True
    ...
    async def upload_file(
        ...
        try:
            async with self._connection.start_sftp_client() as sftp:  # type: ignore[union-attr]
                if check_disk_space:
                    try:
                        # Use string-based parent extraction instead of
                        # Path(remote_path).parent.  On Windows, pathlib
                        # converts ``/`` to ``\\``, which corrupts remote
                        # Unix paths when the resulting string is sent to
                        # the remote SFTP server.
                        remote_dir = remote_path.rsplit("/", 1)[0] if "/" in remote_path else "."
                        vfs = await sftp.statvfs(remote_dir)
                        available = vfs.bfree * vfs.frsize
                        if available < file_size:
                            raise AIOSSHFileDiskFullError(
                                "Insufficient remote disk space", code="DISK_FULL",
    ...
    async def download_file(
        ...
        try:
            async with self._connection.start_sftp_client() as sftp:  # type: ignore[union-attr]
                ...
                start_time = time.monotonic()
                if offset > 0:
                    # asyncssh's SFTPClient.get() has no resume/offset support, so
                    # resume manually: open the remote file, seek past what we
                    # already have locally, and append the rest.
