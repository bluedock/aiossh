...
Current version: **1.1.2**. See [`CHANGELOG.md`](CHANGELOG.md) for a
...
| `orjson` | Faster JSON serialization where available (optional at runtime: `file_manager.py` and `session_replay.py` fall back to the standard-library `json` module if `orjson` is not importable) |
...
│   ├── core.py             # AIOSSH: high-level client facade
│   ├── session.py          # FastSSHSession, SSHConfig
│   ├── pool.py              # ConnectionPool, PoolConfig
│   ├── validators.py       # InputValidator (whitelist-based validation, SSRF/path/command checks)
│   ├── security.py         # SecurityConfig, RateLimiter, AuditLogger, SecureMemory, SecureChannel
...
│   ├── docker_exec.py      # DockerExecSession
│   ├── session_replay.py   # SessionRecorder, SessionReplayer
│   ├── scp_speed.py        # ParallelSCP, TransferProgress
│   ├── decorators.py       # retry, timing (not part of the public __all__ export)
│   ├── exceptions.py       # Exception hierarchy
│   └── py.typed
│
├── examples/                # Six runnable usage examples (see below)
├── tests/                   # Offline unittest suite (29 tests, no network required)
├── README.md
├── CHANGELOG.md
├── LICENSE
└── pyproject.toml
...
| `validate_port(port) -> int` | Validates the 1–65535 range and, on POSIX systems, that the caller has permission to use privileged ports (< 1024). |
...
`tests/` contains a self-contained `unittest` suite (29 tests) covering
...
surface the library depends on.
...
```bash
git clone https://github.com/PyHPDev/aiossh.git
cd aiossh
pip install -e ".[dev]"
ruff check .
mypy src/aiossh
...
MIT License © 2026 PyHPDev. See [`LICENSE`](LICENSE) for the full text.
