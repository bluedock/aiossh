    ...
    async def upload(
        ...
        *,
        ...
        try:
            ...
            # Reassemble on remote (robust)
            parts = " ".join(f"{remote_path}.part{i:04d}" for i in range(num_chunks))
            combine_cmd = (
                f"cat {parts} > {remote_path} 2>/dev/null || true; "
        ...
        *,
        ...
        try:
            stat_result = await self.session.execute(
                f"python3 -c \"import os; print(os.path.getsize('{remote_path}'))\" 2>/dev/null || "
        ...
        # Split on remote
        split_cmd = f"split -b {self.chunk_size} -d {remote_path} {remote_path}.part 2>/dev/null || echo split-failed"
        ...
        async def download_chunk(i: int) -> None:
            ...
            async with semaphore:
                ...
                try:
                    ...
                    chunk_size_bytes = result.get("file_size", 0) or os.path.getsize(local_chunk) if os.path.exists(local_chunk) else 0
                    async with transfer_lock:
                        transferred += chunk_size_bytes
                except Exception:
                    pass  # partial failure tolerance for demo; in prod track errors
        ...
        # Cleanup remote parts
        await self.session.execute(f"rm -f {remote_path}.part* 2>/dev/null || true")
